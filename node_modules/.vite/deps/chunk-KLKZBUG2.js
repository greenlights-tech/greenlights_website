...
var _parseShape = function _parseShape2(shape, forcePath, target) {
  ...
  if (!isString || _selectorExp2.test(shape) || (shape.match(_numExp) || []).length < 3) {
    e = _toArray(shape)[0];
    if (e) {
  ...
  while (--j > -1) {
    ...
    for (i = 6; i < l; i += 6) {
      ...
      if (smooth) {
        smoothData[i - 2] = a;
        smoothData[i + 2] = a2;
        smoothData[i - 1] = _sqrt2(x * x + y * y);
        smoothData[i + 3] = _sqrt2(x2 * x2 + y2 * y2);
      }
...
var _tweenRotation = function _tweenRotation2(start, end, i, linkedPT) {
  var so = this._origin, eo = this._eOrigin, dx = start[i] - so.x, dy = start[i + 1] - so.y, d = _sqrt2(dx * dx + dy * dy), sa = _atan22(dy, dx), angleDif, _short;
  dx = end[i] - eo.x;
  dy = end[i + 1] - eo.y;
...
gsap/MorphSVGPlugin.js:
  ...
  *)
*/
//# sourceMappingURL=chunk-KLKZBUG2.js.map
